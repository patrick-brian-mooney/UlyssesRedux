...
temporary_tags = ['Ulysses itself (by Joyce)'].copy()
...
log_it('\nINFO: the_client is: ')
if patrick_logger.verbosity_level >= 2: dump(the_client)
...
the_line = '<p><a rel="me muse" href="%s">%s</a>' %(new_post_url, the_title)
the_line = the_line + ' (%s):</p>' %  datetime.date.today().strftime("%d %B %Y")
the_line = the_line + '<blockquote><p>%s.</p>' % content_lines[0][3:-4].split('.')[0]
the_line = the_line + '<p>tags: ' + html_tags + '</p></blockquote>\n'
