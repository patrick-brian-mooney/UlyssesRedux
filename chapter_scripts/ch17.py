...
def get_question(num_sents, num_words):
    log_it("    get_question() called", 2)
    log_it(f"      num_sents: {num_sents}; num_words: {num_words}", 3)
    return questions_genny.gen_text(sentences_desired=num_sents, paragraph_break_probability=0)


def get_answer(num_sents, num_words):
    log_it("    get_answer() called", 2)
    log_it(f"      num_sents: {num_sents}; num_words: {num_words}", 3)
...
def write_story():
    ...
    with open(ds.ithaca_stats_path) as stats_file:     # OK, parse the coded structure line
        log_it(f"INFO: successfully opened stats file {ds.ithaca_stats_path}.", 3)
        for structure_line in stats_file:
