...
length_tolerance = 0.4      # ratio by which the generated text can be up to +/- 30% the requested length.
...
articles_files = list((ds.current_run_corpus_directory / '07').glob('*txt'))
for which_file in articles_files:
    mixin_texts_length += os.stat(which_file).st_size

ratio = max(int(round((mixin_texts_length / joyce_text_length) * joyce_ratio)), 1)  # ensure ratio is at least 1 to keep Ulysss from dropping out completely!
...
def get_paragraph(genny, num_sents, num_words):
    ...
    log_it("      get_paragraph() called", 2)
    log_it(f"        num_sents: {num_sents}\n"
           f"        num_words: {num_words}\n"
           f"        chain_length: {genny.chains.markov_length}", 3)
    log_it(f"        looking for a paragraph of {min_len} to {max_len} words", 3)
    ...
    while not ( min_len <= len (ret.split(' ')) <= max_len ):  # Keep trying until it's within acceptable length params
        ...
        log_it(f"          length of generated text is {len(ret.split(' '))} words / {len(ret)} characters", 3)
        log_it(f"            generated sentence was '{ret}'.", 4)

    return ret


def get_headline(num_sents, num_words):
    log_it("    get_headline() called", 2)
...
def write_story():
    ...
    with open(ds.aeolus_stats_path) as statsfile:     # OK, parse the coded structure line
        log_it(f"INFO: successfully opened stats file {ds.aeolus_stats_path}.", 3)
        for structure_line in statsfile:
            log_it(f"  processing line '{structure_line.rstrip()}'.")
